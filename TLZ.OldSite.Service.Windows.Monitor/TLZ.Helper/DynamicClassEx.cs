...
namespace TLZ.Helper
{
    ...
    public class DynamicClassEx
    {
        ...
        /// <returns>返回创建的类实例</returns>
        public static object CreateInstance(string className, List<CustomerPropertyInfo> lcpi)
        {
        ...
        public static object CreateInstance(List<CustomerPropertyInfo> lcpi)
        {
        ...
        public static Type AddProperty(Type classType, List<CustomerPropertyInfo> lcpi)
        {
        ...
        /// <returns>返回处理过的类型的实例。</returns>
        public static Type AddProperty(Type classType, CustomerPropertyInfo cpi)
        {
            List<CustomerPropertyInfo> lcpi = new List<CustomerPropertyInfo>();
            lcpi.Add(cpi);
            //合并先前的属性，以便一起在下一步进行处理。
            MergeProperty(classType, lcpi);
            //把属性加入到Type。
        ...
        public static Type DeleteProperty(Type classType, string propertyName)
        {
            ...
            //合并先前的属性，以便一起在下一步进行处理。
            List<CustomerPropertyInfo> lcpi = SeparateProperty(classType, ls);
            //把属性加入到Type。
            return AddPropertyToType(classType, lcpi);
        }
        ...
        private static void MergeProperty(Type t, List<CustomerPropertyInfo> lcpi)
        {
            CustomerPropertyInfo cpi;
            foreach (PropertyInfo pi in t.GetProperties())
            {
                cpi = new CustomerPropertyInfo(pi.PropertyType.FullName, pi.Name);
                lcpi.Add(cpi);
            }
        }
        ...
        /// <param name="lcpi">要移除的属性列表。</param>
        private static List<CustomerPropertyInfo> SeparateProperty(Type t, List<string> ls)
        {
            List<CustomerPropertyInfo> ret = new List<CustomerPropertyInfo>();
            CustomerPropertyInfo cpi;
            foreach (PropertyInfo pi in t.GetProperties())
            {
        ...
        /// <param name="lcpi">里面包含属性列表的信息。</param>
        private static void AddPropertyToTypeBuilder(TypeBuilder myTypeBuilder, List<CustomerPropertyInfo> lcpi)
        {
            ...
            // 添加属性到myTypeBuilder。
            foreach (CustomerPropertyInfo cpi in lcpi)
            {
                //定义字段。
        ...
        /// <returns>返回处理过的类型的实例。</returns>
        public static Type AddPropertyToType(Type classType, List<CustomerPropertyInfo> lcpi)
        {
            AppDomain myDomain = Thread.GetDomain();
            AssemblyName myAsmName = new AssemblyName();
            myAsmName.Name = "MyDynamicAssembly";
        ...
        public class CustomerPropertyInfo
        {
            ...
            public CustomerPropertyInfo(string type, string propertyName)
            {
                this.type = type;
